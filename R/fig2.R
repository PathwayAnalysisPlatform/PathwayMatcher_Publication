...
proteoformEdgesFile <- "resources/proteoformInternalEdges.tsv.gz"
referenesFile <- "resources/publications.csv.gz"
...
plot <- ggplot() + 
    geom_line(data = referencesDF, aes(x = year, y = n, col = type, linetype = type)) + 
    scale_color_manual(values = c(accessionColor, proteoformAccessionColor, proteoformColor)) + 
    scale_linetype_manual(values = c("dotted", "longdash", "solid")) + 
    scale_y_continuous(name = "# Publications (Cumulative)", expand = c(0, 0), limits = c(0, 1.05 * (nAA + 100))) + 
    scale_x_continuous(name = "Year", expand = c(0, 0)) + 
    geom_text(aes(x = 2016, y = nAA + 100, label = nAA), col = accessionColor, hjust = 1, vjust = 0) + 
    geom_text(aes(x = 2016, y = nPA + 100, label = nPA), col = proteoformAccessionColor, hjust = 1, vjust = 0) + 
    geom_text(aes(x = 2016, y = nPP + 100, label = nPP), col = proteoformColor, hjust = 1, vjust = 0) + 
    theme(
...
png("docs/Fig_2.png", height = 9, width = 16, units = "cm", res = 600)
plot(plot)
dummy <- dev.off()
