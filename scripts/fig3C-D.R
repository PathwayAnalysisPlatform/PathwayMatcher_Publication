...
png("figures/paper/fig_3C.png", height = 12, width = 12, units = "cm", res = 600)
grid.draw(plotGrob)
dummy <- dev.off()
...
png("figures/paper/fig_3D.png", height = 12, width = 12, units = "cm", res = 600)
plot(plot)
dummy <- dev.off()
...
write.table(lowDegreeAccession, gzfile("docs/resources/lowDegreeAccession.gz"), sep = "\t", col.names = T, row.names = F, quote = F)

lowDegreeProteoform <- matchingDF[matchingDF$degreeProteoformLog < 1.5, ]
write.table(lowDegreeProteoform, gzfile("docs/resources/lowDegreeProteoform.gz"), sep = "\t", col.names = T, row.names = F, quote = F)

hubGain <- matchingDF[matchingDF$degreeProteoformLog > 2.5 & matchingDF$degreeAccessionLog < 2.2, ]
write.table(hubGain, gzfile("docs/resources/highDegreeGain.gz"), sep = "\t", col.names = T, row.names = F, quote = F)

hubLoss <- matchingDF[matchingDF$degreeAccessionLog > 2.8, ]
write.table(hubLoss, gzfile("docs/resources/highDegreeLoss.gz"), sep = "\t", col.names = T, row.names = F, quote = F)
